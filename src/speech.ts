...
export function listenOnce({timeoutMs=30000,silenceMs=2500,signal}:ListenOptions={}):Promise<SpeechResult>{
  return new Promise((resolve,reject)=>{
    ...
    const complete=()=>finish(()=>transcript?resolve({text:transcript}):reject(new Error('Keine Sprache erkannt. Bitte erneut versuchen.')));
    ...
    recognition.onresult=(event:any)=>{
      transcript=Array.from(event.results||[]).map((result:any)=>String(result?.[0]?.transcript||'')).join(' ').replace(/\s+/g,' ').trim();
      clearTimeout(silenceTimer);
      if(transcript)silenceTimer=window.setTimeout(complete,silenceMs);
    };
    recognition.onend=()=>{if(settled)return;if(transcript){complete();return}restartTimer=window.setTimeout(start,200)};
